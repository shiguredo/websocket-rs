...
- `src/websocket_extension.rs::Extension::parse_strict` の戻り値型を `Result<Vec<Extension>, String>` から `Result<Vec<Extension>, ExtensionParseError>` に変更し、6 箇所の `Err(format!(...))` をすべて `Err(ExtensionParseError::InvalidValue(format!(...)))` に置換した（issue 方針どおり `InvalidValue` バリアントのみ使用）
- `src/websocket_client_connection.rs` と `src/websocket_server_connection.rs` の呼び出し側 `map_err` を `Display` 経由（`{e}` フォーマット）で `Error::handshake_rejected(...)` に変換するよう更新した
- `skills/shiguredo-websocket/SKILL.md` の `parse_strict` 戻り値型記述を更新した
- 既存 PBT (`pbt/tests/prop_handshake.rs` の `parse_strict` 群) は `result.is_err()` のみ確認しているため型変更で修正不要
