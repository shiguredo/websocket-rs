...
ソース内の RFC 引用コメント 6 箇所の節番号・文脈を、`refs/` の一次資料と一致させる。実装挙動は変えない。
...
| 5 | `websocket_frame.rs:231-237` | 非最小表現の節 | RFC 6455 Section 5.2（ペイロード長の最小表現） |
| 6 | `websocket_server_connection.rs:259` | 9.1 が送信側検証のように読める | 受信側義務である旨をコメントで明示 |
...
- 上記 6 箇所が `refs/` と整合する節番号・文脈になっている
